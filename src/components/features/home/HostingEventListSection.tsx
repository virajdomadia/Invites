  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      ...
      try {
        ...
        // The API returns { data: { events: [...], metadata: {...}, user_info: {...} }, message: "...", status: "success" }
        const innerData = response.data?.data || response.data;
        let events = Array.isArray(innerData) ? innerData : innerData?.events;
        ...
        const validatedEvents = events.filter((event: any) => {
          return event && typeof event === 'object' && 'event_id' in event && 'summary' in event;
        });
...
function HostingEventCard({
  ...
  const handleShareLink = async () => {
    try {
      ...
      // Share failed - error message already shown
    }
  };
